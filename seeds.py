...
db = Session() # Use Session for any CRUD operation

db.add_all([ # Seed users
...
db.commit() # db.add_all only prepares the db to add, need to run commit() to insert data

# insert posts
db.add_all([ # Run posts second to create posts with relation to existing users
...
db.close() # Close db after altering
